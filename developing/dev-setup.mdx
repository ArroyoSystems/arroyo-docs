...
Arroyo is built via Cargo, the Rust build tool. All services are built into a single binary, arroyo-bin, which
can be built with

```
$ cargo build --package arroyo-bin
```
...
This will build the binary in target/debug/arroyo-bin. Running that you should see the available subcommands:

```
$ target/debug/arroyo-bin
...
```
$ target/debug/arroyo-bin cluster
```
...
```
$ cargo run --bin arroyo-bin -- cluster
```
...
* `arroyo-single` -- includes all dependencies needed to run a complete Arroyo cluster, including postgres and
  prometheus.
...
```
$ docker build . -f docker/Dockerfile -t arroyo-single --target arroyo-single
```
