...
The JSON functions provide basic json parsing functions using
 [JsonPath](https://goessner.net/articles/JsonPath/)
  an evolving standard for querying JSON objects.
...
```sql
SELECT get_json_objects('{"a": 1, "b": 2, "c": 3}', '$.a') FROM source;
['1']
```

### get_first_json_object
Returns the first json object the first argument that matches the JsonPath in the second argument.
It is returned as a json string.
```sql
SELECT get_first_json_object('{"a": "a", "b": 2, "c": 3}', '$.a') FROM source;
'\"a\"'
```
