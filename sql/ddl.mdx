...
| `topic`           | The name of the topic to read from or write to. Specific to Kafka connections.|
| `format`          | The format of the data to be deserialized. Defaults to `json`, but also supports `raw_json`, which is a single text field named `value`, and `json_schema_registry`, which trims the first 5 bytes off of each message.  |
| `event_time_field`| The name of the field to use as the event time. If not specified, the event time will be set by the source.|
| `watermark_field` | The name of the field to use as the watermark. If not specified, the watermark will be 1 second before the event time.|
