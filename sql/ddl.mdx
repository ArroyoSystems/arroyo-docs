...
| `event_time_field`| no  | The name of the field to use as the [event time](/concepts#event-time). If not specified, the event time will be set by the source.|
| `watermark_field` | no  | The name of the field to use as the [watermark](/concepts#watermark). If not specified, the watermark will be 1 second before the event time. This will usually be the event_time_field minus some constant interval, e.g. `event_time - interval '10 seconds'`.|
...
These are done using the `GENERATED ALWAYS AS (expression) STORED` syntax.
