...
| `store-deserialization-errors` | Whether to persist deserialization errors to the database | `true` |
...
All services support TLS encryption, which can be configured globally for all services or per-service
by prefixing the key by the service name (for example, `api.tls`).

Key: `tls`

| Name            | Description                       | Required |
|-----------------|-----------------------------------|----------|
| `cert-file`     | Path to the TLS certificate file (PEM format) | Yes |
| `key-file`      | Path to the TLS private key file (PEM format) | Yes |
| `mtls-ca-file`  | Path to the CA certificate for mutual TLS (mTLS) authentication | No |
...
The API service supports multiple authentication modes, configured via the `auth_mode` option:
...
```toml
[api]
auth_mode = { type = "static-api-key", api-key = "your-secret-key" }
```
...
mtls-ca-file = "/path/to/ca-cert.pem"

[api]
auth_mode = { type = "mtls", ca-cert-file = "/path/to/ca-cert.pem" }
```

With mTLS, clients must present a valid certificate signed by the specified CA.
...
Like the API service, the Admin service supports authentication modes. See the [API Authentication](#authentication) section for details.
