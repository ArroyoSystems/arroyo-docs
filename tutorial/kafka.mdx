...
Under the Cluster config box, click "Create new" and enter the following information:
...
<Tip>
    ...
    In SQL this requires setting `'format_options.confluent_schema_registry' = 'true'` in the with clause.
</Tip>
...
INSERT INTO kafka_sink SELECT count(*), store_order_id
FROM orders
GROUP BY store_order_id, hop(interval '2 seconds', interval '10 seconds');
```
