...
We will start by creating a _connection_, which tell Arroyo how to connect to a remote system (in this
case, an HTTP server).
...
The access the tags, we'll need to define our source slightly differently, use the raw json encoder. This
will give us a single column with the raw JSON data.
