...
Arroyo comes with an SSE connector, so we just need create a connection source
that points to the mastodon server. A Connection in Arroyo is an external system
that Arroyo will read or write to.

There are two ways to create a connection in Arroyo. We can create a re-usable connection in the
...
this case `sse` (for server-sent evnets). Also required is `format`, which
...
the `events` field to filter the events that we will read. See all of the
options for the kafka connector in the [connectors docs](/connectors/sse).
