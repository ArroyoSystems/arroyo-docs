...
<Tip>
Arroyo runs a mirror of the mastodon firehose, hosted at
`http://mastodon.arroyo.dev/api/v1/streaming/public`, which does not require
...
The [`extract_json`](https://doc.arroyo.dev/sql/scalar-functions#extract-json) function takes a string value and
...
```sql
CREATE VIEW tags AS (
    SELECT unnest(tags) as tag FROM (
        SELECT extract_json(value, '$.tags[*].name') AS tags
        FROM mastodon)
);
```

We've also added a call to [`unnest`](https://doc.arroyo.dev/sql/select-statements#unnest-operator), which is a
