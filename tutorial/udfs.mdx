...
You can install StreamGen with:

<CodeGroup>
```bash macOS
$ brew install arroyo-systems/tap/streamgen
```
```bash Cargo
...
stream of CLF records, and send them to Arroyo via [Server-Sent Events](/connectors/sse).

```bash
$ streamgen --output sse --port 9995 common-log
```

This will start a server on port 9995 that will respond to SSE requests with a stream
of CLF records. You can test it out by running:

```bash
$ curl -N http://localhost:9995/sse
```
...
fn parse_log(s: String) -> Option<String> {
...
2. UDFs are Rust functions that take some number of arguments and return a
   ...
   case, we're taking a String as input and returning an `Option<String>`, which
...
```sql
select parse_log(data) from logs;
```
...
something about it. For example, we could use Arroyo's [Webhook sink](/connectors/http) to tell
...
pipeline with the "Start Pipeline" command.
