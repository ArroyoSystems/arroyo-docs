...
<img src="/images/dataflow.svg" alt="example of a logical dataflow" height="300"/>
...
<img src="/images/physical_plan.svg" alt="example of a physical dataflow" style={{height: "400px", textAlign: "center"}}/>
...
dataflow is performed via in-memory queues; otherwise the dataflow is perofrmed via Arroyo's network stack which forms
a logical NxN connection between every communicating subtask.

## Timely processing

Arroyo supports timely processing, following the model described in the
[Google Dataflow paper](https://research.google.com/pubs/pub43864.html). A brief overview of the model will be covered
here, but a more in depth explanation can be found in
...
systems, events may be arrive in any order and may be arbitrary delayed. But we still need to be able to compute reuslts,
...
a fixed offset from the last event timestamp, but in the future we will support more sophisticated watermking approaches
...
Arroyo runs pipelines written in SQL. Producing a dataflow from a SQL query involves several steps:

1. First, the query is parsed and planned via [Datafusion](https://github.com/apache/arrow-datafusion)
2. Next, the query plan is compiled into an Arroyo logical dataflow plan
3. Then we generated Rust code representing the operations of that logical plan
4. The Rust code is compiled into a binary that will run on the Arroyo worker nodes
